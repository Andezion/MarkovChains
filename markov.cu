...
MarkovChainCUDA::MarkovChainCUDA(const int states, int agents)
...
static void MarkovChainCUDA::setTransitionMatrix(const std::vector<float>& matrix_host)
{
    CHECK_CUDA(cudaMemcpy(d_transition_matrix, matrix_host.data(),
               num_states * num_states * sizeof(float), cudaMemcpyHostToDevice));
}

static void MarkovChainCUDA::setStates(const std::vector<int>& states_host)
{
...
void MarkovChainCUDA::initRNG(const unsigned long seed) const
{
    int blockSize = 256;
...
void MarkovChainCUDA::step() const
{
    int blockSize = 256;
    int gridSize = (num_agents + blockSize - 1) / blockSize;
